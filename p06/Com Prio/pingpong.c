/*
Projeto 06 com Prioridades - Autores:
Kelvin James
...
task_t *scheduler()
{
    ...
    aux = filaProntas;
    next = aux;
    //Neste loop, achamos a tarefa com prioridade mais alta;
    do
    {
    ...
    //Neste loop envelhecemos todas as demais tarefas
    do
    {
        if(aux != next)
        {
    ...
    while(aux != filaProntas);
    //Voltamos a prioridade dinamica para a estática
    //da tarefa escolhida;
    next->dynamicPrio = next->staticPrio;
    //Remove a tarefa escolhida da fila de prontas e retorna ela para o
    //dispatcher (FIFO);
